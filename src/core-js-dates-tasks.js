...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const offset = new Date(dateEnd) - new Date(dateStart);
  const result = Math.floor(offset / 1000 / 60 / 60 / 24) + 1;
  return result;
}
...
function formatDate(date) {
  ...
  const timeOfDay = hours >= HALF_DAY ? 'PM' : 'AM';
  hours = hours % HALF_DAY || HALF_DAY;
...
function getWeekNumberByDate(date) {
  ...
  const shiftedDayOfWeek = (date.getDay() + 6) % 7;
  givenDate.setDate(givenDate.getDate() - shiftedDayOfWeek + 3);

  const milliseconds = givenDate.valueOf();

  givenDate.setMonth(0, 1);
  if (givenDate.getDay() !== 4)
    givenDate.setMonth(0, 1 + ((4 - givenDate.getDay() + 7) % 7));
  const weeks = Math.ceil(
    (milliseconds - givenDate.valueOf()) / 1000 / 60 / 60 / 24 / 7
  );
  return weeks + 1;
}
...
function getNextFridayThe13th(date) {
  ...
  let found = false;
  while (!found) {
    const thisMonth13 = new Date(currentYear, currentMonth, 13);
    if (thisMonth13.getDay() === 5) {
      found = true;
      return thisMonth13;
    }
    currentMonth += 1;
